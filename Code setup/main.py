...
from changeparameters import readparameter
from SimilarityMetrics import medpyDC, medpyHD, medpyRVD, findfixedmask2d, findfixedmask3d, findtransformedmask
from combineatlasses import createmodel2d, createmodel3d, validatemodel, runmodel

movingnr = 120
fixednr = 125
slicenr = 30
parameter_file = 'parameters_samplespace_MR.txt'
...
atlasset = patients[0:5]
optimizeset = patients[5:10]
...
#modelnr = createmodel3d(atlasset,optimizeset,parameter_file,threshold=0.3)
#modelnr = createmodel2d(atlasset,optimizeset,parameter_file,threshold=0.3)

#modelnr = 8

#model validation command
#validatemodel(modelnr,validationset) #For now only works with 3d model

unknownset = [127,128]
#model run command
#runmodel(modelnr,unknownset) #For now only works with 3d model